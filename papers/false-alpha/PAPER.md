...
**Contributions.** The statistical tools are established — the deflated Sharpe is López de Prado & Bailey's, the Reality Check is White's, and the finding that LLM trading edge evaporates under rigorous evaluation is peer-reviewed (FINSABER, KDD'26; §5). Building on them, our contributions are:
1. A multiple-testing deflation for the minimum-regime-performance statistic (§4.9). Scoring a strategy by its worst regime is the minimum-regime-performance (MRP) criterion of Alexander & Fabozzi (2026), who use it as a strategy-durability diagnostic but do not correct it for search. The standard deflated Sharpe, by contrast, is the expected max of N *single* Sharpes — the wrong null for a min-over-regimes score. We derive and Monte-Carlo-validate the correct envelope E[maxₙ minₖ Z] and show the standard DSR over-deflates it by ~2.4×; the real-market zero survives the corrected, far-weaker bar. To our knowledge, deflating a min-over-regimes best-of-N statistic for multiple testing is new.
2. A robustness-dividend identity (§4.9): worst-of-k *independent* regimes ≈ a single-Sharpe search over only N′≈7 strategies — robustness *is* an implicit multiple-testing correction, which collapses as regimes correlate (real S&P 500 at ρ_reg≈0.67 → N′≈84), explaining (§4.3) why explicit deflation is load-bearing on real data and not on the adversarial null.
...
2. The contribution is therefore a precision/recall characterization of anti-overfit gating: this gate has (empirically) ~zero false-discovery rate across the noise and small-edge controls and seven real markets (S&P 500, DJIA, NASDAQ, a 15-stock cross-sectional panel, FX, gold, crypto), at the cost of failing to detect edges below a worst-regime Sharpe of ≈2–3 at N=1,000. We measure the precision side directly (`fdr.py`): on the strong planted market, every strategy the gate admits on one sample also survives an *independent* sample — FDR ≈ 0 even at N=1,000 (a pre-registered prediction that FDR would *rise* with N, refuted: family-wise control delivers ~zero false discoveries here, and the price is recall, not precision). That tradeoff *itself* tightens as N grows — the engine of the whole phenomenon.
...
**The recall floor in closed form (`mde.py`).** Inverting the gate gives the *minimum detectable edge* — the smallest true worst-regime Sharpe that survives a search of size N (at power 0.5). Under the corrected worst-of-k deflation (§4.9) it rises 0.69 (N=10) → 1.15 (100) → 1.48 (1,000) → 1.62 (3,000); under the standard single-Sharpe DSR it is ~2× higher (2.83 at N=1,000). Either way, at LLM scale a strategy needs a *genuine* worst-regime Sharpe well above 1 just to be provable — and the best any real market offers here is ≈ 0.85 (Appendix A), comfortably below the floor. This is the empirical power surface as a formula, and it tells the practitioner the exact price of search scale: each ~3× step in N adds roughly +0.15–0.25 to the Sharpe you must clear.
...
- **Worst-of-regimes scoring and maximum-Sharpe inference.** Scoring a strategy by its lowest risk-adjusted return across regimes is the minimum-regime-performance (MRP) criterion of Alexander & Fabozzi (2026), used there as a durability diagnostic; inference on the *maximum* in-sample Sharpe over a set of candidates is developed by Pav (2019). We build directly on both — our contribution is the multiple-testing deflation of the MRP-style min-over-regimes best-of-N statistic (§4.9), which neither provides.

**What is new here.** The statistical machinery is established: the multiple-testing gate is White (2000) / STW (1999) / Bailey–López de Prado (2014); STW(1999) already reported a data-snooping-adjusted zero-survivor result on S&P 500 futures (Reality-Check Sharpe p-values ≈ 0.99), transaction-cost-aware and OOS, on parameter-optimized rule grids; and the "discovery threshold rises with N" curve is the leading-order asymptotic of BLdP Eq. 1, whose Exhibit 2 already plots the threshold rising with N (so our "power surface" is the *empirical instantiation* of a known result, not a new theorem). What the surveyed literature does not contain, and what we contribute, is narrow but real: (i) a multiple-testing deflation of the minimum-regime-performance statistic (Alexander & Fabozzi 2026) — the expected max of N of the min over k regimes, E[maxₙ minₖ Z], which is ~2.4× smaller than the single-Sharpe DSR envelope the prior literature (including the maximum-Sharpe inference of Pav 2019) uses; deflating the *robustness* statistic correctly (rather than mis-applying the single-Sharpe haircut) is, to our knowledge, new, and it tightens the real-market margin from −1.5 to −0.3 while keeping the zero (§4.9); (ii) a cross-generator design — random vs evolutionary-creative vs a live LLM — through *one fixed gate*, isolating the generator's effect; (iii) planted-edge / pure-noise controls that *calibrate* the gate, absent from both the econometrics canon (one rule family) and the LLM-alpha work (real data only); (iv) a cautionary negative importing the GWAS effective-number-of-independent-tests machinery (Nyholt 2004; Li & Ji 2005; Galwey 2009) — it is anti-conservative for the best-of-N max statistic and the bootstrap must arbitrate (§4.9); and (v) the LLM diversity-ceiling finding (160 distinct from 1,000 draws). In one line: we re-confirm a known negative (STW; FINSABER) with a more formal, generator-controlled apparatus, and contribute a correct worst-of-regimes deflation, a cross-generator and planted-edge calibration, a cross-disciplinary cautionary negative, and the LLM diversity ceiling.
...
- **Seed variance — now quantified (`seed_variance.py`).** Redrawing each pool under 8 independent seeds, the real-market survivor count is 0 ± 0 under every seed at every N (bestOOS bands ±0.00–0.07), and the planted control is positive under every seed for the creative generator at every N and for random at N≥300 (creative N=1,000: 63 ± 7) — the single-seed curves are representative, not lucky draws. Residual: daily Sharpes are √252-annualized with no serial-correlation adjustment, which can flatter the "annual Sharpe ~1.0" rhetoric.
...
The author thanks Antoine Cully, who pointed him to the Extract-QD line of work on the winner's curse in quality-diversity search; Joel Lehman and Jean-Baptiste Mouret for helpful correspondence on selection inflation, novelty search, and evaluator overfitting; Robert Lange for discussion of open-ended LLM program search; and John Yang for discussion of held-out and chronological evaluation of coding agents. These exchanges shaped how the evaluation in this paper is framed. No acknowledgment implies endorsement of this work; all views and any errors are the author's own.
...
- Alexander, N., & Fabozzi, F. J. (2026). Measuring Strategy-Decay Risk: Minimum Regime Performance and the Durability of Systematic Investing. *Journal of Portfolio Management* (forthcoming). arXiv:2604.08356.
...
- Flageat, M., Huber, J., Hélénon, F., Doncieux, S., & Cully, A. (2025). Extract-QD Framework: A Generic Approach for Quality-Diversity in Noisy, Stochastic or Uncertain Domains. *Proc. Genetic and Evolutionary Computation Conference (GECCO 2025)*.
...
- Pav, S. E. (2019). Conditional Inference on the Asset with Maximum Sharpe Ratio. *arXiv:1906.00573*.
...
- **Worst-of-regimes deflation (§4.9):** standard DSR over-deflates the min-over-k statistic ~2.4×; corrected bar still gives 0 on real (best deflated ≈ −0.3). Robustness dividend: worst-of-3 at N=1,000 ≈ single-Sharpe search over N′≈7 (independent regimes) — robustness *is* deflation, collapsing to N′≈84 on real S&P (ρ_reg≈0.67) when regimes correlate.
