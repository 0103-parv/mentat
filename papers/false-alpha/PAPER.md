...
We tried a *second* analytic shortcut (`envelope_departure.py`): calibrate an empirical effective‑N per generator by matching the realized best‑of‑N standardized in‑sample Sharpe on a null market to the i.i.d.‑Gaussian envelope E[max of N]. It also proved **unreliable** — the in‑sample Sharpe standardization var = (1+½SR²)/T assumes i.i.d. returns, but strategy returns are autocorrelated (positions persist) and cost‑laden, so the true standard error is larger and the implied M_emp swings by an order of magnitude with the scaling/demeaning choice. The only weakly‑robust signal is that the **live LLM has the lowest realized max** at fixed N (most redundant search, consistent with its 160‑distinct ceiling), but we do not lean on it. **Two parametric effective‑N shortcuts, two failures: there is no reliable analytic effective‑N for this problem, which is precisely why the distribution‑free block bootstrap (§4.6) is load‑bearing rather than a mere check.**
