...
\end{verbatim}}
\caption{The four conditions differ only in $J$: A one frozen $u$; B one co-tuned $u$; C ten co-tuned $u_j$ sharing the trace; D ten frozen $u_j$.}
\label{fig:algo}
\end{figure}
...
\begin{table}[t]
\caption{Calibration gap (reported $-$ true) by condition across the sweep, seed-averaged over 24 reseeds (full columns incl.\ true quality and held-out scores in \texttt{ablation\_cotuned\_results.json}).}
\label{tab:sweep}
\small
\begin{tabular}{@{}rcccc@{}}
\toprule
...
\textbf{Sensitivity: directions are parameter-independent, multipliers are not.}
\label{sec:sensitivity}
A 12-cell one-factor sweep around the baseline (co-tuning rate $\eta\in\{0.1,0.25,0.5,1.0\}$; the \emph{interpolating} update $u \mathrel{+}= \eta(\bar c-u)$ in place of accumulation; $K\in\{10,30,100\}$; ensemble size $\in\{3,10,30\}$; refit fraction $\in\{0.1,0.2,0.5\}$; 8 seeds $\times$ 6 runs per cell) finds all four pre-registered directions hold in 100\% of seeds in \emph{every} cell. The magnitudes move exactly as one should expect: the co-tuned/static inflation ratio ranges from $1.6\times$ (gentlest, $\eta{=}0.1$) to $7.4\times$ ($\eta{=}1.0$), and the co-tuned-ensemble/independent-ensemble ratio from $3.2\times$ to $30.5\times$. The headline $4\times$/$13\times$ are therefore one point in a robust family, not the phenomenon itself; the phenomenon---co-tuning inflates beyond a static judge, shared-trace ensembling barely helps, independence closes the gap and selects better candidates---survives every parameterization tested, including the gentle one (\texttt{sensitivity\_cotuned.py}).

\textbf{Do cheap mitigations suffice?} Two standard fixes, added as sweep cells, help but do not substitute for independence: \emph{drift regularization} (shrinking the rubric halfway back to its initialization each round) leaves a gap of $81.1$---$1.8\times$ the static judge, ${\sim}8\times$ the independent ensemble---and \emph{periodic judge refresh} every 2 rounds neutralizes the runaway only back to the static-judge floor ($46.9$ vs $45.5$), which itself inflates ${\sim}4.5\times$ the independent ensemble. Refresh \emph{every} round does work ($4.0$)---but a judge replaced before it can absorb any trace is precisely a trace-independent judge: the mitigation succeeds exactly insofar as it reinstates independence, which is the paper's point.
...
\textbf{Live instantiation: a real LLM judge rewriting its own rubric.} We instantiate co-tuning with a real LLM (generator and judges; one-line predictor expressions over five features, hidden truth $y=2x_1-x_2+\varepsilon$, \emph{objective} quality computed locally as held-out $R^2$; 8 seeds $\times$ 6 rounds $\times$ batch 8, every completion cached). The co-tuned condition is the practice the paper models: after each round the judge \emph{rewrites its own scoring rubric} given the round's candidates, its scores, and the selected winner. Controls: the same rubric held fixed, and an independent ensemble of three differently-phrased rubrics never exposed to the trace. The shipped champion is then scored by the condition's own final judge state and by a held-out three-rubric panel used in no loop. Pre-registered results: \textbf{(P1, 8/8 seeds)} the co-tuned judge's self-inflation exceeds the fixed judge's (mean $6.9$ vs $3.5$ on the 0--10 scale)---after six rounds of rubric iteration the judge awards its own champion $9.9/10$ on average while the held-out panel scores the same candidate $3.0/10$; \textbf{(P2, 6/8)} the independent ensemble has the smallest gap ($3.0$); \textbf{(P3, 7/8)} the independent ensemble also selects the objectively best candidates (mean $R^2$ $-0.16$ vs the co-tuned $-0.49$). All conditions' champions have negative $R^2$: every judge, of every provenance, confidently scores candidates that are objectively poor---but only the co-tuned judge converges to near-certainty about them. An earlier, weaker pilot (exemplar-appending instead of rubric rewriting, 2 seeds) failed to separate co-tuned from fixed and is retained in the repository as an instructive null: co-tuning requires the judge's \emph{criteria} to absorb the trace, not merely its examples. Scripts: \texttt{llm\_rubric\_iteration.py}, \texttt{llm\_judge\_instantiation.py}.

\section{Limitations and Next Step}
The testbed is synthetic and linear by design---provenance is the only manipulated variable, which is what makes the ablation clean, but effect \emph{sizes} here are not predictions for any real system. Co-tuning is one operationalization (taste refit toward round winners); prompt-iteration on real LLM judges may be gentler or harsher. The live run confirms the co-tuning amplification with a real LLM judge at modest scale (8 seeds, one domain, 0--10 rating scale); the live effect ($\sim$2$\times$ the fixed judge's gap) is smaller than the synthetic testbed's ($\sim$4$\times$), P2/P3 hold as majorities rather than unanimously, and richer domains (code, proofs) remain untested. The mechanism is evaluator-agnostic; extending the live grid is mechanical.
